    ...
    while True:
        ...
        for index, coin in enumerate(sorted_coins, start=1):
            ...
            if price_change_24h > 0:
                price_change_24h = f"{Fore.GREEN}{price_change_24h}%{Style.RESET_ALL}"
            else:
                price_change_24h = f"{Fore.RED}{price_change_24h}%{Style.RESET_ALL}"

            print(f"{index}. Name:{Style.BRIGHT} {name}{Style.RESET_ALL}, Price: ${current_price} USD, Volume: {total_volume}, Max Available: {max_supply}, Circulating Supply: {circulating_supply}, 24h Change: {price_change_24h}")
